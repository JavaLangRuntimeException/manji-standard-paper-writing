...
- `mspw-discussion`: Discussionのメカニズム解釈を簡潔に反映
- `mspw-writing-principles`: 曖昧語の回避、比較対象の明示、引用の正確な解釈
