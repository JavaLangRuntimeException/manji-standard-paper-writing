...
この skill はメイン会話で動く Discussion 章のオーケストレーター。**メイン会話は本文を書かない**:

1. **執筆・修正** → `discussion` subagent に作業指示書(目的・対象範囲・確定/未確定事項)を Agent tool で dispatch する。subagent は起動時にこの SKILL.md を読み込む
2. **OQ の routing** → subagent が返す `[OQ-n]`(不明点)は `paper-researcher` へ、研究上の判断はユーザーへ(AskUserQuestion でまとめて)
3. **レビュー** → `paper-reviewer1`(高重要度なら `paper-reviewer2` も)に依頼し、所見の反映は再び `discussion` subagent へ
...
- `results-orchestrator`: Resultsで事実を述べ、Discussionでは解釈
- `design-implications-orchestrator`: DiscussionとDesign Implicationsは切り分ける
- `writing-principles-orchestrator`: 引用の正確な解釈、曖昧語の回避
