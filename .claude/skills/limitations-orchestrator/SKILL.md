---
name: limitations-orchestrator
description: Orchestrator skill for the Limitations section - delegates writing to the limitations subagent (this file is the rulebook the subagent loads at startup), routes open questions to paper-researcher and reviews to paper-reviewer1/2; the main conversation never writes prose itself. Use this skill whenever the user is writing or revising the Limitations, Limitations and Future Work, or Future Directions section of a paper. Triggers include "Limitations", "Future Work", "limitations", "将来研究", "今後の課題", "制約", "将来の方向性". Also trigger when a reviewer asks the authors to "discuss generalizability" or "address sample limitations". This skill helps the user acknowledge limitations honestly (avoiding both over-apology and hiding weaknesses) and frame them as motivated future directions rather than pure weaknesses.
---
...
この skill はメイン会話で動く Limitations / Future Directions 章のオーケストレーター。**メイン会話は本文を書かない**:

1. **執筆・修正** → `limitations` subagent に作業指示書(目的・対象範囲・確定/未確定事項)を Agent tool で dispatch する。subagent は起動時にこの SKILL.md を読み込む
2. **OQ の routing** → subagent が返す `[OQ-n]`(不明点)は `paper-researcher` へ、研究上の判断はユーザーへ(AskUserQuestion でまとめて)
3. **レビュー** → `paper-reviewer1`(高重要度なら `paper-reviewer2` も)に依頼し、所見の反映は再び `limitations` subagent へ
...
- `conclusion-orchestrator`: Limitations の論旨と Conclusion の主張の整合性
- `writing-principles-orchestrator`: 誠実な記述、曖昧表現の回避
