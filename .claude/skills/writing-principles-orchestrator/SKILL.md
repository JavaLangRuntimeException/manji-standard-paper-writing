---
name: writing-principles-orchestrator
description: Orchestrator skill for cross-cutting writing checks - dispatches the writing-principles checker subagent (this file is the rulebook it loads at startup) and routes fixes to the section-writer subagents; the main conversation neither checks nor edits prose itself. Cross-cutting English academic writing principles that must be applied across EVERY section of a paper. This is a companion skill — use it in addition to section-specific skills (abstract-orchestrator, introduction-orchestrator, etc.) whenever the user is writing ANY part of an academic paper. Triggers on any paper-writing request in English, plus explicit mentions of "proofread", "polish", "check my writing", "grammar", "wording", "英文校正", "推敲", "文章チェック". This skill enforces: (1) replacing vague superlatives ("highest", "best") with comparatives when the comparison set is implicit, (2) precise interpretation of cited works, (3) explaining unfamiliar technical terms, (4) preserving the main claim when editing, and (5) balancing redundancy against reviewer-navigation needs.
---
...
**このSkillは他の章skill（abstract-orchestrator 等）と併用**してください。

## この skill の役目(オーケストレーション)

この skill はメイン会話で動く横断原則チェックのオーケストレーター。**メイン会話は検査も修正もしない**:

1. **チェック** → `writing-principles` subagent(チェッカー)にスコープ(ファイル/章、通常 or 投稿前 mechanical pass)を Agent tool で dispatch する。チェッカーは起動時にこの SKILL.md を読み込む
2. **修正反映** → 所見は該当章のライター subagent(`abstract`〜`conclusion`)へ
3. **外部知識が要る判定** → `paper-researcher` へ
4. フルパイプラインは `/paper-orchestrator` へ

以下は横断原則の本体(チェッカーと全ライターの規範)。
...
この原則Skillは、全ての章skill（abstract-orchestrator〜conclusion-orchestrator）と併用されることを想定。
